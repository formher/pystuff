...
## Defining config file path and name
configpath = currPath + '/config.json'
...
for url in listofurls:
    ...
    try:
        ...
        print('Error with {} URL has been occured:> {}'.format(url, err))
        logging.warning(err)
        pass
    ...
    """ Currently this takes only top level rank. Include all the ranks in the future. 
    E.g. .update({"Rank":ranktemp[len(ranktemp)-1]}) """
    ...
    if os.path.exists(finalcsvresult):
        ...
        os.makedirs(currPath + '\\results', exist_ok=True)
        csv_open = open(finalcsvresult, 'a') ## Add newline='' for windows
        csv_writer = csv.writer(csv_open)
